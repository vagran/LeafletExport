 ...
 * }
 */
export default async function LeafletExport(map, crop = 1.0) {
    return new LeafletExporter(map, crop).Export()
}
...
class LeafletExporter {
    ...
    async Export() {
        ...
        return {
            image: await new Promise(resolve => this.#canvas.toBlob(resolve)),
    ...
    async _FetchTile(tilePoint, tilePos, layer, tileLayer) {
        let imgIndex = `${tilePoint.x}:${tilePoint.y}:${this.#zoom}`
        let image = new Image()
        image.crossOrigin = "Anonymous"
        let url
